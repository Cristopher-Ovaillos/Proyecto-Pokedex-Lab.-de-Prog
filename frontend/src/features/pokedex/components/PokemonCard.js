  ...
  return (
    <TouchableOpacity className={styles.card.touchable} onPress={onPress}>
      <View className={styles.card.container}>
        <Text className={styles.card.id}>{formatedId}</Text>
        ...
        <Text className={styles.card.name}>{pokemon.name}</Text>

        <View className={styles.card.typeContainer}>
          {pokemon.types.map(({ type }) => (
            <View key={type.name} className={styles.card.typePill} style={{backgroundColor: TYPE_COLORS[type.name]}}>
              <Text className={styles.card.typeText}>{type.name}</Text>
            </View>
          ))}
        </View>
