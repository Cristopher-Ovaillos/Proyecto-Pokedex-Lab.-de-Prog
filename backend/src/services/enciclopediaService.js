...
class EnciclopediaService {
  ...
  async getMoveById(id) {
    try {
      ...
      const move = await enciclopediaRepository.findMoveById(moveId);

      if (!move) {
        throw new Error("NOT_FOUND_ERROR: Movimiento no encontrado");
      }
    ...
    try {
      ...
      if (method) {
        ...
        if (!metodosValidos.includes(method.toLowerCase())) {
          throw new Error(
            `VALIDATION_ERROR: Método de aprendizaje '${method}' no válido`,
          );
        }
      }
      ...
      if (category) {
        ...
        if (!categoriasValidas.includes(category.toLowerCase())) {
          throw new Error(
            `VALIDATION_ERROR: Categoría '${category}' no válida`,
          );
        }
      }
  ...
  async getMovesList(
    ...
    try {
      ...
      if (
        parsedMinAccuracy &&
        (parsedMinAccuracy < 0 || parsedMinAccuracy > 100)
      ) {
        throw new Error(
          "VALIDATION_ERROR: La precisión mínima debe estar entre 0 y 100",
        );
      }

      if (
        parsedMaxAccuracy &&
        (parsedMaxAccuracy < 0 || parsedMaxAccuracy > 100)
      ) {
        throw new Error(
          "VALIDATION_ERROR: La precisión máxima debe estar entre 0 y 100",
        );
      }
